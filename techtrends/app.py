...
def post(post_id):
    ...
    if post is None:
        # REQ: A non-existing article is accessed and a 404 page is returned.
        app.logger.error(f"Non-existing article is retrieved with post_id={post_id}!")
        return render_template('404.html'), 404
    else:
        # REQ: An existing article is retrieved. The title of the article should be recorded in the log line.
...
@app.route('/about')
def about():
    # REQ: The "About Us" page is retrieved.
...
def create():
    if request.method == 'POST':
        ...
        if not title:
            ...
            # REQ: A new article is created. The title of the new article should be recorded in the logline.
